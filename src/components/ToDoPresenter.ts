...
export class ItemPresenter {
	protected itemTemplete: HTMLTemplateElement;
	...
	constructor(
		...
		this.itemTemplete = document.querySelector('#todo-item-template') as HTMLTemplateElement;
	...
	renderView() {
		const itemList = this.model.items
			.map((item: IItem) => {
				const todoItem = new this.viewItemConstructor(this.itemTemplete);
				todoItem.on('copy', this.handleCopyItem.bind(this));
				todoItem.on('delete', this.handleDeleteItem.bind(this));
				todoItem.on('edit', this.handelEditItem.bind(this));
